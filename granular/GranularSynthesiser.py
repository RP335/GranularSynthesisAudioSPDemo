...
class GranularSynthesiser:
    ...
    def setParameters(self, grainSize, sizeRandomness, density, densityRandomness, gainRandomness, noiseGain, gain):
        """
    ...
    def updateParameters(self):
        ...
        if (self.gainRandomness != 0):
            drift = int(np.random.uniform(-(self.gainRandomness / 2), self.gainRandomness / 2))
        else:
            drift = 0

        self.currentGain = self.currentGain + drift
    ...
    def blendSounds(self, signalA, signalB, durationSamples, numGrainsA=2, numGrainsB=2, weight=0.5):
        ...
        return self.generateSignal(blendProfile, durationSamples)


    # ********************************** SERGIO **********************************
    def morphSound(self, signalA, signalB, duration, numGrainsA, numGrainsB, morphFactor):
        # Extract grains
        grainProfileA = self.extractGrain(signalA, numGrainsA)
        grainProfileB = self.extractGrain(signalB, numGrainsB)
        ...
        return self.generateSignal(morphProfile, duration)
